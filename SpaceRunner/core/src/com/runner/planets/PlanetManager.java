	...
	public void touchControllers(boolean pause){
		for(int i = 0; i <5 ; i++){
			if(Gdx.input.isTouched(i)){
				...
				if(touch.overlaps(planetLvl.getRectangleResumeButton())){
					planetLvl.getRunner().setPause(false);
					planetLvl.setGameS(1);
				}
				
				if(touch.overlaps(planetLvl.getRectangleMenuButton())){
					planetLvl.setGameS(5);
				}
				
				if(touch.overlaps(planetLvl.getRectangleExitButton())){
					Gdx.app.exit();
				}
			}
		}
